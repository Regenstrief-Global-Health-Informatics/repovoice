...
export const transcribeWithWhisper = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    const { audioBase64, mimeType, filename, apiKey, language } = data;
...
export const transcribeWithGrok = createServerFn({ method: "POST" })
  .validator(cloudInput)
  .handler(async ({ data }) => {
    const { audioBase64, mimeType, filename, apiKey, language, diarize } =
      data;
    ...
    if (!res.ok) {
      const errText = await res.text();
      throw new Error(
        `Grok STT failed (${res.status}): ${errText.slice(0, 400)}`,
      );
    }
